...
def get_scorecard_links(url :str) -> tuple:
    ...
    league_year = url1.split('/')[-2].split('-')[-2]
...
def get_all_match_results(results_links :list) -> None:
    '''
    Given the url for a league year, extract a list of urls with match results,
    for each match in the league extract results from html table,
    tidy rows and columns, dropping redundant data,
    write each table to csv file with appropriate labels.
    '''
...
get_all_match_results(get_scorecard_links(url1))
